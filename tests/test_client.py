...
identity = SungrowIdentificationResult(
    "ABC123", sh10rt, OutputType.THREE_PHASE_3P4L, []
)


@pytest.mark.asyncio
async def test_accessors():
    client = SungrowClient(AsyncModbusTcpClient("127.0.0.1", 502), identity)
...
@pytest.mark.asyncio
async def test_get():
    mb_client = AsyncModbusTcpClient("127.0.0.1", 502)
    ...
    client = SungrowClient(mb_client, identity)
